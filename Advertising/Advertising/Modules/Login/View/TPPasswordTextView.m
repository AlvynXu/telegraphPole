...
//        UITextField *textField = [[UITextField alloc] initWithFrame:self.bounds];
        UITextField *textField = [[UITextField alloc] initWithFrame:CGRectMake(0, 0, 220, 50)];
...
- (void)setElementCount:(NSInteger)elementCount {
    ...
    for (int i = 0; i < self.elementCount; i++)
    {
        ...
        pwdTextField.font = kBoldFont(16);
...
//        pwdTextField.layer.borderColor = kHexColor(0xFF8A8A8A).CGColor;
        [self insertSubview:pwdTextField belowSubview:self.textField];
        pwdTextField.backgroundColor = kHexColor(0xFFEBEBEB);
...
- (void)showKeyboard {
    ...
    for (UITextField *text in self.dataSource) {
        
//        text.layer.borderColor = kMainColor.CGColor;  // 选中时的高亮颜色
    }
}
...
- (void)hideKeyboard {
    ...
    for (UITextField *text in self.dataSource) {
        ...
        text.layer.borderColor = kHexColor(0xFF8A8A8A).CGColor;
    }
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    [self showKeyboard];
//    self.elementBorderColor = kMainColor;  // 选中时的高亮颜色
    [self setNeedsDisplay];
}
